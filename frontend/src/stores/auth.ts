...
export const useAuthStore = defineStore('auth', () => {
  ...
  function checkAuth(): void {
    ...
    const savedProvider = localStorage.getItem(AUTH_PROVIDER_KEY)
    ...
    if (savedToken && savedUser) {
      try {
        ...
        if (isSupabaseAuthMode() && savedProvider === 'supabase') {
          syncSupabaseSession().catch((error) => {
            console.error('Failed to sync Supabase session on init:', error)
          })
          return
        }
  ...
  function setAuthFromResponse(response: AuthResponse): void {
    ...
    localStorage.setItem(AUTH_PROVIDER_KEY, 'backend')
  ...
  async function setSupabaseSession(session: Session): Promise<User> {
    const mappedUser = await mapSupabaseSessionToUser(session)
  ...
  async function logout(): Promise<void> {
    if (isSupabaseAuthMode() && localStorage.getItem(AUTH_PROVIDER_KEY) === 'supabase') {
      try {
  ...
  async function refreshUser(): Promise<User> {
    ...
    if (isSupabaseAuthMode() && localStorage.getItem(AUTH_PROVIDER_KEY) === 'supabase') {
      ...
      return await setSupabaseSession(session)
    }
