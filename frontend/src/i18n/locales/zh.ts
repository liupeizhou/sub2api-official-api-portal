export default {
  ...
  home: {
    ...
    goToDashboard: '进入控制台',
    // 新增：面向用户的价值主张
    heroSubtitle: '一个密钥，体验多个中国大模型',
    heroDescription: '一站式接入 GLM、通义千问、DeepSeek、MiniMax、阶跃星辰等中国模型',
    tags: {
      subscriptionToApi: '邮箱随机码登录',
      stickySession: '中国模型路由',
      realtimeBilling: '用量可见'
    },
    // 用户痛点区块
    painPoints: {
      title: '你是否也遇到这些问题？',
    ...
    features: {
      ...
      multiAccount: '稳定可靠',
      multiAccountDesc: '智能调度多个官方 API 上游，自动切换和负载均衡，告别频繁报错。',
      balanceQuota: '用多少付多少',
  ...
  keys: {
    ...
    useKeyModal: {
      ...
      antigravity: {
        description: '为 MiniMax / 阶跃星辰分组配置 API 访问。请根据您使用的客户端选择对应的配置方式。',
        claudeCode: 'GLM 兼容客户端',
        geminiCli: 'StepFun 兼容客户端',
        claudeNote:
      ...
      gemini: {
        description:
          '将以下环境变量添加到您的终端配置文件或直接在终端中运行，以配置 DeepSeek 兼容访问。',
        modelComment: '例如：deepseek-v4-flash 或 deepseek-v4-pro',
        note: '这些环境变量将在当前终端会话中生效。如需永久配置，请将其添加到 ~/.bashrc、~/.zshrc 或相应的配置文件中。'
      },
      opencode: {
        title: 'OpenCode 配置示例',
