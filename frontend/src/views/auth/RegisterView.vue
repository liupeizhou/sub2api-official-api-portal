<template>
  <AuthLayout>
    <div class="space-y-6">
      ...
      <div
        v-if="!supabaseModeEnabled && !registrationEnabled && settingsLoaded"
      ...
      <form v-else @submit.prevent="handleRegister" class="space-y-5">
        ...
        <div v-if="supabaseModeEnabled && supabaseOtpSent">
          <label for="supabase-register-code" class="input-label">邮箱随机验证码</label>
          <div class="relative">
            <div class="pointer-events-none absolute inset-y-0 left-0 flex items-center pl-3.5">
              <Icon name="key" size="md" class="text-gray-400 dark:text-dark-500" />
            </div>
            <input
              id="supabase-register-code"
        ...
        <div v-if="!supabaseModeEnabled && invitationCodeEnabled">
        ...
        <div v-if="!supabaseModeEnabled && promoCodeEnabled">
        ...
        <!-- Turnstile Widget -->
        <div v-if="!supabaseModeEnabled && turnstileEnabled && turnstileSiteKey">
          <TurnstileWidget
            ref="turnstileRef"
      ...
      <div v-if="!supabaseModeEnabled && showOAuthLogin" class="space-y-3 pt-1">
...
onMounted(async () => {
  ...
  if (supabaseModeEnabled) {
    ...
    promoCodeEnabled.value = false
    invitationCodeEnabled.value = false
    turnstileEnabled.value = false
    loginAgreementEnabled.value = false
    agreementAccepted.value = true
    siteName.value = appStore.siteName || appStore.cachedPublicSettings?.site_name || '老刘给老孙的验证站'
...
function validateForm(): boolean {
  ...
  if (supabaseModeEnabled) {
    ...
    if (supabaseOtpSent.value && !supabaseOtpCode.value.trim()) {
      errors.supabaseCode = '请输入邮件里的 6 位随机验证码。'
      isValid = false
    } else if (supabaseOtpSent.value && !/^\d{6}$/.test(supabaseOtpCode.value.trim())) {
...
function resetSupabaseEmail(): void {
