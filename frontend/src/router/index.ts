...
const routes: RouteRecordRaw[] = [
  ...
  },
  {
    ...
    name: 'SupabaseAuthCallback',
    component: () => import('@/views/auth/SupabaseCallbackView.vue'),
    meta: {
      requiresAuth: false,
      title: 'Supabase Auth Callback'
    }
  },
  {
...
router.beforeEach(async (to, _from, next) => {
  ...
  if (requiresAuth && isSupabaseAuthMode() && authStore.isAuthenticated) {
    try {
      await authStore.refreshUser()
    } catch (error) {
      console.warn('Failed to refresh Supabase auth state before navigation:', error)
    }
  }
