...
type SecurityConfig struct {
	...
	OfficialAPIProviders             OfficialAPIProviderPolicyConfig `mapstructure:"official_api_providers"`
...
type OfficialAPIProviderPolicyConfig struct {
	// Enabled makes the admin account pool accept only official API provider credentials.
	// This is the production mode for commercial API resale/aggregation deployments.
	Enabled bool `mapstructure:"enabled"`
	// AllowedAccountTypes lists account credential types allowed when Enabled is true.
	// Defaults to official/provider-owned API surfaces only.
	AllowedAccountTypes []string `mapstructure:"allowed_account_types"`
	// RequireAPIKey requires credentials.api_key for apikey/upstream accounts.
	RequireAPIKey bool `mapstructure:"require_api_key"`
	// RequireBaseURLForUpstream requires credentials.base_url for upstream accounts.
	RequireBaseURLForUpstream bool `mapstructure:"require_base_url_for_upstream"`
}
...
func setDefaults() {
	...
	viper.SetDefault("security.official_api_providers.enabled", true)
	viper.SetDefault("security.official_api_providers.allowed_account_types", []string{
		"apikey",
		"upstream",
		"bedrock",
		"service_account",
	})
	viper.SetDefault("security.official_api_providers.require_api_key", true)
	viper.SetDefault("security.official_api_providers.require_base_url_for_upstream", true)
...
func (c *Config) Validate() error {
	...
	if c.Security.OfficialAPIProviders.Enabled {
		allowed := normalizeStringSlice(c.Security.OfficialAPIProviders.AllowedAccountTypes)
		if len(allowed) == 0 {
			return fmt.Errorf("security.official_api_providers.allowed_account_types is required when official API provider mode is enabled")
		}
		for _, typ := range allowed {
			switch typ {
			case "apikey", "upstream", "bedrock", "service_account":
			default:
				return fmt.Errorf("security.official_api_providers.allowed_account_types contains unsupported type %q", typ)
			}
		}
		c.Security.OfficialAPIProviders.AllowedAccountTypes = allowed
	}
