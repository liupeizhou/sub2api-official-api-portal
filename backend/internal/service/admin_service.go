...
func (s *adminServiceImpl) CreateAccount(ctx context.Context, input *CreateAccountInput) (*Account, error) {
	...
	if err := enforceOfficialAPIProviderPolicy(s.cfg, input.Type, input.Credentials); err != nil {
		return nil, err
	}
...
func (s *adminServiceImpl) UpdateAccount(ctx context.Context, id int64, input *UpdateAccountInput) (*Account, error) {
	...
	if len(input.Credentials) > 0 {
		nextCredentials = MergePreservingSensitiveCreds(account.Credentials, input.Credentials)
	}
	if strings.TrimSpace(input.Type) != "" || len(input.Credentials) > 0 {
		if err := enforceOfficialAPIProviderPolicy(s.cfg, nextType, nextCredentials); err != nil {
			return nil, err
		}
	}
...
func (s *adminServiceImpl) BulkUpdateAccounts(ctx context.Context, input *BulkUpdateAccountsInput) (*BulkUpdateAccountsResult, error) {
	...
	if len(input.Credentials) > 0 && officialAPIProviderPolicyEnabled(s.cfg) {
		...
		for _, account := range accounts {
			...
			if err := enforceOfficialAPIProviderPolicy(s.cfg, account.Type, merged); err != nil {
